...
namespace RevenuePlanner.Helpers
{
    public class Common
    {
        ...
        public static bool CheckTacticIsAllowedAndIsEditable(int planTacticId)
        {
            ...
            if (lstEditableTacticIds.Count > 0)
            {
                if (!lstEditableTacticIds.Contains(planTacticId))
                {
                    returnValue = false;
                }
            }
        ...
        public static bool CheckTacticIsAllowedAndIsNotRestricted(int planTacticId)
        {
            ...
            if (!isCustomRestrictionSet)
            {
                returnValue = false;
            }
            else if (lstRestrictedTacticIds.Count > 0)
            {
                if (!lstRestrictedTacticIds.Contains(planTacticId))
                {
                    returnValue = false;
                }
            }
        ...
        public static List<int> GetEditableTacticList(Guid userId, Guid clientId, List<int> lstTactic, bool isDisplayForFilter = true)
        {
            ...
            try
            {
                using (MRPEntities objDbMrpEntities = new MRPEntities())
                {
                    if (lstTactic.Count() > 0 && Common.IsCustomFeildExist(Enums.EntityType.Tactic.ToString(), clientId)) //todo : able to move up
                    {
                        //// Get list customFieldEntity List for given tactic list
                        string DropDownList = Enums.CustomFieldType.DropDownList.ToString();
                        string EntityTypeTactic = Enums.EntityType.Tactic.ToString();
