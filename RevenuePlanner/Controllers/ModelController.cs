    ...
    public class ModelController : CommonController
    {
        ...
        public ActionResult ModelZero()
        {
            ...
            try
            {
                ...
                if (Sessions.User != null)
                {
                    ...
                    if (lstAllowedBusinessUnits.Count > 0)
                        lstAllowedBusinessUnits.ForEach(g => lstAllowedBusinessUnitIds.Add(Guid.Parse(g)));
                    if (AuthorizeUserAttribute.IsAuthorized(Enums.ApplicationActivity.UserAdmin) && lstAllowedBusinessUnitIds.Count == 0)// Sessions.IsSystemAdmin || Sessions.IsClientAdmin || Sessions.IsDirector)
                    {
        ...
        public JsonResult GetModelList(string listType)
        {
            ...
            var lstModel = objModelList.Select(p => new
            {
                ...
                isOwner = (Sessions.User.UserId == p.CreatedBy || Common.IsBusinessUnitEditable(p.BusinessUnitId)) ? 0 : 1,/*added by Nirav Shah  on 14 feb 2014  for 256:Model list - add delete option for model and -	Delete option will be available for owner or director or system admin or client Admin */
        ...
        public JsonResult GetTacticDatabyid(int id)
        {
            ...
            */
            ...
            // commented by Dharmraj , ticket #592 Tactic type data model
            ...
            {
                id = p.TacticTypeId,
                clientid = Sessions.User.ClientId,//clientid = p.ClientId,
                ...
                IsDeployedToModel = p.IsDeployedToModel // added by dharmraj for #592 : Tactic type data model
        ...
        public PartialViewResult DetailTacticData(int id = 0, int ModelId = 0)
        {
            ...
            try
            {
                ...
                tm.ClientId = Sessions.User.ClientId;//mtp.ClientId;
                ...
                tm.ModelId = mtp.ModelId;
                // added by Dharmraj, ticket #592 : Tactic type data model
                ViewBag.IsDeployed = mtp.IsDeployedToModel;
                ...
                ViewBag.TacticTypeStageId = mtp.StageId;
        ...
        public ActionResult DeleteTactic(int id = 0, string UserId = "")
        {
            ...
            if (ttModel != null)   // Modified by Sohel Pathan on 19/06/2014 for PL ticket #536
            {
        ...
        public ActionResult SaveTactic(string Title, string Description, int? StageId, double ProjectedStageValue, double ProjectedRevenue, int TacticTypeId, string modelID, bool isDeployedToIntegration, bool isDeployedToModel)
        {
            try
            {
                ...
                {
                    var existTactic = db.TacticTypes.Where(t => (t.ModelId == ModelId) && t.Title.ToLower() == Title.ToLower() && t.TacticTypeId != TacticTypeId && (t.IsDeleted == null || t.IsDeleted == false)).ToList();
        ...
        public void OtherModelEntries(int OldModelID, bool IsVersion, string Title, string BusinessUntiID, bool IsBenchmarked, ref int newModelId)
        {
            using (MRPEntities mrp = new MRPEntities())
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    ...
                    {
                        Guid tmpGuid = Guid.Parse(BusinessUntiID);
                    ...
                    newModel.Status = Enums.ModelStatusValues.Single(s => s.Key.Equals(Enums.ModelStatus.Draft.ToString())).Value;
        ...
        //var oldModel_Audience_Event = mrp.Model_Audience_Event.Where(m => m.ModelId == OldModel).ToList();
        ...
        //var oldModel_Audience_Inbound = mrp.Model_Audience_Inbound.Where(m => m.ModelId == OldModel).ToList();
        ...
        //var oldModel_Audience_Outbound = mrp.Model_Audience_Outbound.Where(m => m.ModelId == OldModel).ToList();
        ...
        /// <summary>
        /// Added by Mitesh Vaishnav for PL ticket #659 
        /// action for save integration instance of model
        /// </summary>
        /// <returns></returns>
        ...
        public ActionResult IntegrationOverview(int id = 0)
        {
           ...
           /*creating List of model integration instance */
            var lstInstance = (from i in db.IntegrationInstances
                               where ModelInstances.Contains(i.IntegrationInstanceId)
                               select i).ToList();
            foreach (var key in Enums.IntegrationActivity.Keys)
            {
                ...
                {
                    var objInstance = lstInstance.Where(ins => ins.IntegrationInstanceId == objModel.IntegrationInstanceId).FirstOrDefault();
                ...
                else if (key.ToString() == "IntegrationInstanceIdINQ")
                {
                    ...
                    var objInstance = lstInstance.Where(ins => ins.IntegrationInstanceId == objModel.IntegrationInstanceIdINQ).FirstOrDefault();
                    objIntSelection.Setup = Enums.IntegrationActivity[key].ToString();
                    if (objInstance != null)
                    {
                        objIntSelection.Instance = objInstance.Instance;
                ...
                else if (key.ToString() == "IntegrationInstanceIdMQL")
                {
                    ...
                    var objInstance = lstInstance.Where(ins => ins.IntegrationInstanceId == objModel.IntegrationInstanceIdMQL).FirstOrDefault();
                    objIntSelection.Setup = Enums.IntegrationActivity[key].ToString();
                    if (objInstance != null)
                    {
                        objIntSelection.Instance = objInstance.Instance;
                ...
                else if (key.ToString() == "IntegrationInstanceIdCW")
                {
                    ...
                    var objInstance = lstInstance.Where(ins => ins.IntegrationInstanceId == objModel.IntegrationInstanceIdCW).FirstOrDefault();
                    objIntSelection.Setup = Enums.IntegrationActivity[key].ToString();
                    if (objInstance != null)
                    {
                        objIntSelection.Instance = objInstance.Instance;
        ...
        public ActionResult IntegrationSelection(int id = 0)
        {
            ...
            {
                ViewBag.Flag = chekckParentPublishModel(id);
                ViewBag.IsOwner = db.Models.Where(a => a.IsDeleted.Equals(false) && a.ModelId == id && a.CreatedBy == Sessions.User.UserId).Any();  
            }
            else
            {
                ViewBag.IsOwner = true;
            ...
            var lstInstance = db.IntegrationInstances.Where(inc => inc.ClientId == Sessions.User.ClientId && inc.IsDeleted==false).Select(inc => new
            {
