    ...
    for (var i = 0; i < columns.length; i++) {
        ...
        if (col.name == "text") {
            ...
            if ("add" == col.name && i == columns.length - 1) {
                if (item.type == "Plan") {
                    ...
                    if (item.Permission != null && item.Permission == true)
                    {
					    if (item.id == "000") { // bind only add icon for new bank row -- #2587
					        ...
					        value = "<div id='Plan' class='grid_Search' title='View' data-original-title='View' onclick=DisplayEditablePopup('" + item.PlanId + "','Plan')><i class='fa fa-external-link-square'></i></div><div id='" + item.type + "' class='grid_add' Name='" + item.id + "' data-toggle='tooltip' title='Add' aria-label='" + _item + "' Permission='" + item.Permission + "' onclick='DisplayPopUpMenu(this,event)'><i class='fa fa-plus-circle'></i></div> <div id='" + item.type + "' onclick='javascript:AddRemoveEntity(this)'  class='honeycombbox-icon-gantt calender-view-honeycomb' title= 'Select' altId='" + item.id + "' csvid='" + item.type + '_' + item.PlanId + "'  ColorCode= '" + item.colorcode + "' TacticType= '" + item.TacticType + "' OwnerName= '" + item.guidUserId + "' taskname='" + _item + "' Permission='" + item.Permission + "'>" + "</div>  ";
						}
                    }
                    else
                    {
					    if (item.id != "000") {
					        value = "<div id='Plan' class='grid_Search' title='View' data-original-title='View' onclick=DisplayEditablePopup('" + item.PlanId + "','Plan')><i class='fa fa-external-link-square'></i></div><div id='" + item.type + "'  class='honeycombbox-icon-gantt calender-view-honeycomb' onclick='javascript:AddRemoveEntity(this)' title='Select' altId='" + item.id + "' csvid='" + item.type + '_' + item.PlanId + "'  ColorCode= '" + item.colorcode + "' TacticType= '" + item.TacticType + "' OwnerName= '" + item.guidUserId + "' taskname='" + _item + "' Permission='" + item.Permission + "'></div>"
						    }
                        else
                            value = "";
                    }
                ...
                if (col.name == "add") {
                    value = "<div class='grid_add' data-toggle='tooltip' title='Add'></div>";
                } else {
                    if (col.template)
