...
DECLARE @AuthDBName NVARCHAR(255) = '' -- Name Of Database from where User's List have to fetch
DECLARE @DBName NVARCHAR(255) = '' -- Name of Database in which User_Permission need to set
DECLARE @ClientId NVARCHAR(MAX) = '' -- List of ClientId in Comma Separated  -- e.g. '''C251AB18-0683-4D1D-9F1E-06709D59FD53'',''B14648DA-6A91-43E1-8FE2-0997C9180F55''' In this format

SET @CustomQuery='	
	DELETE FROM '+@DBName+'.dbo.User_Permission WHERE UserId IN (SELECT Usr.ID
	...
	BEGIN
		...
		SELECT @DashboardId, NULL, GETDATE(), ''View'', NULL, Usr.ID, Usr.ID
		FROM '+@AuthDBName+'.dbo.User_Application UA
		INNER JOIN '+@AuthDBName+'.dbo.[Application] App ON (App.ApplicationId = UA.ApplicationId AND App.Code = ''MRP'' AND App.IsDeleted = 0)
		INNER JOIN '+@AuthDBName+'.dbo.[User] Usr ON (Usr.UserId = UA.UserId AND Usr.IsDeleted = 0)
		WHERE UA.IsDeleted = 0
