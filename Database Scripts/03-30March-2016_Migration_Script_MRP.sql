...
*/
  ...
  DECLARE @inst1 as int = (SELECT IntegrationTypeId FROM [dbo].[IntegrationType] where Title = 'WorkFront')
  if exists(select * from  [dbo].[GameplanDataType] where IntegrationTypeId = @inst1 AND ActualFieldName = 'Campaign End Date')
  begin 

DELETE m
FROM [dbo].[IntegrationInstanceDataTypeMapping] m
INNER JOIN [dbo].[GameplanDataType] t
  on t.ActualFieldName = 'Campaign End Date' and t.IntegrationTypeId = @inst1 and t.GameplanDataTypeId = m.GameplanDataTypeId
 delete from  [dbo].[GameplanDataType]  where IntegrationTypeId = @inst1 AND ActualFieldName = 'Campaign End Date'
  end

 if not exists(select * from  [dbo].[GameplanDataType] where IntegrationTypeId = @inst1 AND ActualFieldName = 'CampaignEndDate')
 insert into [dbo].[GameplanDataType] values(@inst1,'Plan_Campaign', 'CampaignEndDate', 'Campaign End Date', 0,0,0)
